...
{
    public class MySqlBizRepository<TModel, TCondition> : database.core.MysqlRepository<TModel>, IBizRepository<TModel, TCondition> where TModel : MBaseModel, new() where TCondition : MBaseModel, new()
    {
        public string TableName { get; }
        /// <summary>
        /// 构造函数
        /// </summary>
        public MySqlBizRepository(IConfiguration configuration, string tableName) : base(configuration)
        {
            TableName = tableName;
        }
        ...
        public async Task<int> BactchDeleteByIdListAsync(List<string> idList)
        {
            ...
            // 执行删除操作
            return await DapperTool.ExecuteNonQueryAsync($" delete from  `{TableName}` where logical_id in ('" + sqlWehre + "')");
        }
        ...
        public async Task<int> BactchLogicDeleteByIdListAsync(List<string> idList)
        {
            ...
            // 执行删除操作
            return await DapperTool.ExecuteNonQueryAsync($" update `{TableName}` set `is_delete`=1  where logical_id in ('{sqlWehre}')");
        }
        ...
        {
            string sqlWhere = GetSqlWhereByModelFunc(model, "")?.ToString();
            return sqlWhere;
        }
